...
namespace Turbo.Plugins.James
{
    public class PartyMatchingWebsiteMonitorPlugin : BasePlugin, IKeyEventHandler
    {
	   ...
	   private string[,] WebAds = new string[3, 2] { {string.Empty, string.Empty}, {string.Empty, string.Empty}, {string.Empty, string.Empty} };		// (광고내용, 배틀태그) * 3개
		...
		private void listView_SelectedIndexChanged(object sender, EventArgs e)
		{
         ...
         public void OnKeyEvent(IKeyEvent keyEvent)
         {
			...
			if(InputBox("인벤 파티 모집 검색어", "Or : comma/space, And : ( Or )", ref value) == DialogResult.OK)
			{
				...
				WebBBSearchTimer.Interval = 1000;		// first in 1 sec and the Search Web bulletin boards every WebBBSearchInterval
