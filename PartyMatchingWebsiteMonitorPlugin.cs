	   ...
	   private int checkedrButton = 1;
		...
		public DialogResult listView_Doit(string title, string content)
		{
			...
			for (int i = 0; i < WebAds.GetLength(0)-1; i++)
		      ...
		      catch {} 	// for unusual exception handling
		...
		private void listView_SelectedIndexChanged(object sender, EventArgs e)
		{
         ...
         public void OnKeyEvent(IKeyEvent keyEvent)
         {
			...
			ClickTimer.Enabled = true;
				...
				if (WaitFriendScreen("Root.NormalLayer.BattleNetFriendsList_main.LayoutRoot.OverlayContainer.FriendsListContent"))		// 친구 창이 떴으면
				{
	            		Cursor.Position = new Point(1560, 900);	//친구 창 화면에서 "친구 추구" 버튼의 위치
		          		Process.Start("D:\\Game\\click.exe");	// Just click the button
		          		Thread.Sleep(300);
		          		if (WaitFriendScreen("Root.TopLayer.BattleNetSocialDialogs_main.LayoutRoot.DialogSendRealIdRequest"))		// 친구 추가 창이 떴으면
		          		{
          ...
          public bool WaitFriendScreen(string UiName)		// Wait till Friend Screen opens
          {
          		bool exist = false;
