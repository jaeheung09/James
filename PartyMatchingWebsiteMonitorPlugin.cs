	   ...
	   private int checkedrButton = 1;
		...
		private void listView_SelectedIndexChanged(object sender, EventArgs e)
		{
         ...
         public void OnKeyEvent(IKeyEvent keyEvent)
         {
            ...
            {
            	if (Hud.Render.GetUiElement("Root.NormalLayer.BattleNetFriendsList_main.LayoutRoot.OverlayContainer.FriendsListContent").Visible)
		  		Console.Beep(500, 150);
		  	
            }            
          }
          
          public bool WaitFriendScreen()		// Wait till Friend Screen opens
          {
          		bool exist = false;
          		for (int i = 0; i < 4; i++)		// to avoid an exception
          		{
	          		if (Hud.Render.GetUiElement("Root.NormalLayer.BattleNetFriendsList_main.LayoutRoot.OverlayContainer.FriendsListContent").Visible);
	          		{
	            		exist = true;
