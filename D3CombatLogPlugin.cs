...
{
    public class D3CombatLogPlugin : BasePlugin, IKeyEventHandler, IInGameTopPainter, INewAreaHandler, IInGameWorldPainter
    {
		...
		public void ReadEditLine(Object source, System.Timers.ElapsedEventArgs e)
			...
			if (match.Success)	// in the edit line, should type "/combatlog/ or /combatlong n/" <- the n is the log interval.
			{
				...
				if (!isOK)
	        		...
	        		{
	        			if (culture == "ko")
	        				Hud.Sound.Speak("로그 간격이 " + defaultVal + Convert.ToString(iVal) + "초 로 설정 되었습니다..");
	        			else
		...
		public void AllowScanMonsters(Object source, System.Timers.ElapsedEventArgs e)
        	...
        	{
        		if (monitoring)
        			FrameRateWorld = CalculateFrameRateWorld();		// PaintWorld frame
        	}

        	public void PaintTopInGame(ClipState clipState)
        	{
        		if (monitoring)
				FrameRateTop = CalculateFrameRateTop();		// frame before checking clipstate
		...
		public void MonitoringResource(Object source, System.Timers.ElapsedEventArgs e)
			...
			if (match.Success)
			{
				...
				if (Success && GpuUse > 0 && GpuUse <= 100)
				{
					...
					GpuText = "Gpu Usuage : " + GpuVal + " %" + Environment.NewLine;
				}
			}
			...
			if (FrameRate < FPSBad && cLatency < LatencyWarning && CpuUse < CpuWarning && RamUse > RamWarning)
			{
				if (FPSWarningCnt++ > 5 && Hud.Sound.LastSpeak.TimerTest(4000))
				{
					...
					if (culture == "ko")
						Hud.Sound.Speak("FPS가 비정상적으로 낮습니다. 백그라운드 프로그램을 확인하시고 필요시 종료하세요. 특히 웹페이지 확인!");
					else
						Hud.Sound.Speak("Low FPS warning! Check your background application and close it. particularly check a web page!");
				}
			}
			...
			var FPSTopVal = Convert.ToString(FrameRateTop);
			var FPSWorldVal = Convert.ToString(FrameRateWorld);
			//var FPSText =      "FrameRate  : " + FPSVal + " FPS";
			var FPSText =      "FrameRate  : " + FPSVal + "/" +  FPSTopVal + "/" +  FPSWorldVal + " FPS";
			...
			MonitoredLog = CpuVal + " %, " + GpuVal + " %, " + RamVal + " MB, " + LatencyVal + " ms, " + FPSVal + "/" + FPSTopVal + "/" +FPSWorldVal + " FPS";
		}
